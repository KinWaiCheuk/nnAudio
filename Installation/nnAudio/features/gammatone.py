...
class Gammatonegram(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if trainable_gammatone:
            # Making everything nn.Parameter, so that this model can support nn.DataParallel
            gammatone_basis = nn.Parameter(gammatone_basis, requires_grad=trainable_gammatone)
    ...
    def extra_repr(self) -> str:
        return "Gammatone filter banks size = {}, trainable_gammatone={}".format(
            (*self.gammatone_basis.shape,), self.trainable_gammatone, self.trainable_STFT
        )
