...
class Model(torch.nn.Module):
    def __init__(self):
        ...
        self.STFT_layer = Spectrogram.MelSpectrogram(sr=44100, n_fft=n_fft, n_mels=n_mels, hop_length=HOP_LENGTH, pad_mode='constant', center=True, device=device)
...
import pickle
with open(f'./result/nnAudio_result_Mel-{n_mels}', 'wb') as f:
    pickle.dump(nnAudio_result,f)
    
torch.save(model.state_dict(), f'./weight/nnAudio_result_Mel-{n_mels}')
