...
parser.add_argument("n_fft", type=int,help=" n_fft for STFT:")
...
class Model(torch.nn.Module):
    def __init__(self):
        ...
        self.STFT_layer = Spectrogram.MelSpectrogram(sr=44100, n_fft=n_fft, n_mels=n_mels, hop_length=HOP_LENGTH, pad_mode='constant', trainable_mel=True, center=True, device=device)
    ...
    def forward(self,x):
        x = torch.relu(self.STFT_layer(x[:,:-1]))
        x = torch.log(x+1e-5)
...
nnAudio_result['processing_time'] = 0

import pickle
with open(f'./result/trainable_nnAudio_result_Mel-{n_mels}', 'wb') as f:
    pickle.dump(nnAudio_result,f)
    
torch.save(model.state_dict(), f'./weight/trainable_nnAudio_result_Mel-{n_mels}')
