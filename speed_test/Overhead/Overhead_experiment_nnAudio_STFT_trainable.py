...
class Model(torch.nn.Module):
    def __init__(self):
        ...
        f_kernal = 128//factor
        self.STFT_layer = Spectrogram.STFT(sr=44100, n_fft=n_fft, hop_length=HOP_LENGTH, center=True, device=device, trainable=True)
...
import pickle
with open(f'./result/trainable_nnAudio_result_STFT-{n_fft}', 'wb') as f:
    pickle.dump(nnAudio_result,f)
    
torch.save(model.state_dict(), f'./weight/trainable_nnAudio_result_{n_fft}')
