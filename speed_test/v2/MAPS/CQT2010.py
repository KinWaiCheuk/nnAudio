...
if args.device in ["CPU", "GPU"]:
    ...
    spec_layer = Spectrogram.CQT2010v2(sr=44100, n_bins=84, bins_per_octave=24, fmin=55, device=device)
    timing = []
    for e in range(20):
        t_start = time.time()
    ...
    data.to_csv(Path(__file__).parent /f'./result/CQT2010_torch_{args.device}')
