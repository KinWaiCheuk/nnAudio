...
if args.device in ["CPU", "GPU"]: 
    ...
    data = pd.DataFrame(timing,columns=['t_avg'])
    data['Type'] = 'librosa'
    data.to_csv(Path(__file__).parent /f'./result/librosa_Mel')
    ...
    print('Extracting Melspectrograms using tf.signal...')
    timing = []
    with tf.Session() as sess:
        for e in range(20):
            ...
            print(time_used)
            timing.append(time_used)
    print('Test result for Melspectrograms using tf.signal')
    print("mean = ",np.mean(timing))
    print("std = ", np.std(timing))
    ...
    data.to_csv(Path(__file__).parent /f'./result/Mel_tensorflow_GPU')    
    ...
    for e in range(20):
        t_start = time.time()
        specgram = torchaudio.transforms.MelSpectrogram(sample_rate=44100, n_fft=2048, hop_length=512).to(device)(y_torch)
        time_used = time.time()-t_start
        print(time_used)
        timing.append(time_used)
    ...
    data.to_csv(Path(__file__).parent /f'./result/Mel_torchaudio_GPU')
